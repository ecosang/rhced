...
library(lubridate)
# figure for 
df=read_csv("data/bldg1/raw_data/unit_data_b17_2018-02-18_2018-02-24.csv")%>%select(timestamp,ahu,heatpump,net,T_out,operation)#%>%mutate(timestamp=with_tz(timestamp,tzone="America/Indianapolis"))
...
g2+g3+g4+plot_layout(heights = c(0.8,0.1,0.3))
...
days_vec
df30=df%>%mutate(timestamp=floor_date(timestamp,unit="30min"))
...
ggsave("outputs/figures/timeseries_30min.png",dpi=300,width=12,height=6)
